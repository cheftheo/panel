...
<head>
    ...
    <title>Panel <?php echo $serverName ?> - Search</title>
					...
					<div class="card-body">
						...
						<div class="alert alert-info">
							...
							<table class="table">
								...
								<tbody>
								<?php
								$selectSearch = "SELECT * from `users` WHERE `username` LIKE '".mysqli_real_escape_string($db,$_POST['search'])."%' LIMIT 50";
								$rezultatul = $db->query($selectSearch);
								if ($rezultatul->num_rows > 0) {
									while($row = $rezultatul->fetch_assoc()) {
										?>
										<tr>
											<td><a id="1" href="<?=$serverURL;?>profile.php?user=<?php echo $row["id"] ?>"><?php echo $row["username"] ?></a></td>
					...
					<div class="card-body">
						...
						<div class="col-4 mx-auto">
							<form action="" method="POST">
								...
								<input type="text" style = "color: white; border:none; background-color: rgb(35,35,35); padding: 10px;" name="search" id="search" class="form-control">
								<input type="submit" value="Search" class="btn btn-primary float-right mt-1">
								<input type="hidden" name="_token" value="9zCYTgPteg0MSuAW8mQ7vjM8gFBw5RqzjcOx0SfU">
							</form>
						</div>
					</div>
				</div>
