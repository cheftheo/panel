...
if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
	...
	$results = $db->query("SELECT COUNT(*) FROM houses");
				...
				<div class="card">
					<div class="card-body">
						...
						<table class="table color-table dark-table">
							...
							<tbody>
							<?php
								$sql = "SELECT * from houses ORDER BY ID ASC LIMIT $page_position, $item_per_page";
								$result = $db->query($sql);
									...
									while($row = $result->fetch_assoc()) {
										?>
											<tr>
												...
												<?php } ?>
												<?php if($row["Owner"] == "") { ?>
												<td><span class="text-danger"><?=calculeazabani($row["Value"])?>$</span></td>
												<?php }else{ ?>
